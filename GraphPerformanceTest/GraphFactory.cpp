...
/*
 * this function creates a graph with fixed size and hierarchy, but with random player names
 *										root
...
Graph GraphFactory::create()
{
	...
	{
		vertex_t c = boost::add_vertex(VertexProperty{ id++, Node_type::COUNTRY, std::make_shared<GraphCountry>("Country " + std::to_string(i + 1)) }, g);
		boost::add_edge(root, c, g);

		for (int j = 0; j < numberLeagues; ++j)
		{
			vertex_t l = boost::add_vertex(VertexProperty{ id++, Node_type::LEAGUE, std::make_shared<GraphLeague>("League " + std::to_string(j + 1)) },g);
			boost::add_edge(c, l, g);

			for (int x = 0; x < numberTeams; ++x)
			{
				vertex_t t = boost::add_vertex(VertexProperty{ id++, Node_type::TEAM, std::make_shared<GraphTeam>("Team " + std::to_string(x + 1)) },g);
				boost::add_edge(l, t, g);

				for (int y = 0; y < numberPlayers; ++y)
				{
					auto nameTuple = ng.createName();
					vertex_t p = boost::add_vertex(VertexProperty{ id++, Node_type::PLAYER, std::make_shared<GraphPlayer>(std::get<0>(nameTuple), std::get<1>(nameTuple)) }, g);
					boost::add_edge(t, p, g);
				}
			}
		}
	}
